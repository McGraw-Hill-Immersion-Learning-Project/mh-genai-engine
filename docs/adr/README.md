...
| **ADR-003** (RAG Framework) | **Custom pipeline (no LangChain):** `app/core/rag/` — `retriever.py`, `generator.py`, `pipeline.py`, `prompts/` (`template_strategy.py`, `registry.py`, `templates/*.md`). Ingestion remains `app/core/ingestion/`. |
