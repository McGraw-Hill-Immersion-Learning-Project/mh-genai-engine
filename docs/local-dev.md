...
- **In-process RAG** lives under `app/core/rag/` (`Retriever`, `Generator`, `LessonOutlinePipeline`, `prompts/`). After ingestion, chunks carry `chapter`, `section`, `title`, and **`source_key`** (ingest filename) in metadata. Those fields feed **vector query filters** and **citations**. The semantic search query is built from the learning objective plus audience and duration (not from chapter labels alone).
- **Metadata filters (`LessonOutlineRequest`):** `chapter` / `section` / `subSection` scope retrieval as documented in OpenAPI. **`book`** is a case-insensitive substring on **`metadata.title` *or* `metadata.source_key`** — use this when PDF embedded title is empty (common for OER); e.g. `book: "eepsam"` matches chunks stored under `eepsam.pdf`.
- **HTTP — generate:** `POST /generate/lesson-outline` (`app/api/generate.py`) uses **`LessonOutlinePipeline`** with **`app/deps.py`**. You need **Postgres + indexed chunks**, embedding config, and an LLM key when not using a test override. Optional **`LOG_LEVEL=DEBUG`** prints full prompt text for `app.*` loggers.
- **HTTP — regenerate:** `POST /generate/lesson-outline/regenerate` takes **`previousOutline`**, **`refinementInstructions`**, and optional **`chunkIds`**. Echo **`chunkId`** from a prior response to load the same passages by id; omit or send an empty array for embedding-based retrieval (no chapter/book filters on that path). Stale ids → **422**. See **`docs/api/openapi.yaml`** and **`docs/api/CHANGELOG_API.md`** (regenerate: v0.6.0+).
...
- **Grounding:** Responses include **`citations`** (one per retrieved chunk; each has **`chunkId`**, **`snippet`**, etc.) and model text may use **`<grounded ref="i">`** matching `citations[i]` (including **`slideOutline`** for `ppt`).
- **Request timeout:** Optional **`REQUEST_TIMEOUT_SECONDS`** in `.env` (default **120**; **`0`** = off). Exceeded → **504** for any route (`app/middleware/timeout.py`).
